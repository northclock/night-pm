...
export function registerCalendarTools(server: McpServer, projectPath: string) {
  const filePath = getProjectFile(projectPath, 'calendar.json');

  server.tool('calendar_list_events', 'List all calendar events', {}, async () => {
    try {
      const events = await readJsonFile<CalendarEvent>(filePath);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify(events, null, 2) }],
      };
    } catch (err) {
      return {
        content: [{ type: 'text' as const, text: `Error listing events: ${err}` }],
        isError: true,
      };
    }
  });
  ...
  server.tool(
    ...
    async ({ title, description, start, end, allDay }) => {
      try {
        ...
        return {
          content: [
            { type: 'text' as const, text: `Event "${title}" added with ID ${newEvent.id}` },
          ],
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `Error adding event: ${err}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ id, ...updates }) => {
      try {
        ...
        if (idx === -1) {
          return {
            content: [{ type: 'text' as const, text: `Event with ID ${id} not found` }],
            isError: true,
          };
        }
        ...
        await writeJsonFile(filePath, events);
        return {
          content: [{ type: 'text' as const, text: `Event "${events[idx].title}" updated` }],
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `Error updating event: ${err}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ id }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Event deleted` }],
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `Error deleting event: ${err}` }],
          isError: true,
        };
      }
    },
