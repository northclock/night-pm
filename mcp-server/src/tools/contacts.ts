...
export function registerContactTools(server: McpServer, projectPath: string) {
  const filePath = getProjectFile(projectPath, 'contacts.json');

  server.tool('contact_list', 'List all contacts', {}, async () => {
    try {
      const contacts = await readJsonFile<Contact>(filePath);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify(contacts, null, 2) }],
      };
    } catch (err) {
      return {
        content: [{ type: 'text' as const, text: `Error listing contacts: ${err}` }],
        isError: true,
      };
    }
  });
  ...
  server.tool(
    'contact_add',
    'Add a new contact. Checks for duplicates by name first — if a match is found, returns the existing contact instead of creating a duplicate.',
    {
    ...
    async ({ name, title, info, relatedContacts }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error adding contact: ${err}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    'contact_search',
    'Search contacts by name (partial, case-insensitive match). Use this BEFORE contact_add to check if a contact already exists.',
    {
    ...
    async ({ query }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error searching contacts: ${err}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ id, info, ...updates }) => {
      try {
        ...
        if (idx === -1) {
          return {
            content: [{ type: 'text' as const, text: `Contact with ID ${id} not found` }],
            isError: true,
          };
        }
        ...
        return {
          content: [
            { type: 'text' as const, text: `Contact "${contacts[idx].name}" updated` },
          ],
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `Error updating contact: ${err}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ id }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error deleting contact: ${err}` }],
          isError: true,
        };
      }
    },
