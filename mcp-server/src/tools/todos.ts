...
export function registerTodoTools(server: McpServer, projectPath: string) {
  ...
  server.tool(
    ...
    async ({ status }) => {
      try {
        let todos = await readJsonFile<Todo>(filePath);
        if (status) {
  ...
  server.tool(
    ...
    async ({ title, description, dueDate, status }) => {
      try {
        const todos = await readJsonFile<Todo>(filePath);
        ...
        return {
          content: [
            { type: 'text' as const, text: `Task "${title}" added with ID ${newTodo.id}` },
          ],
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `Error adding task: ${err}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ id, ...updates }) => {
      try {
        ...
        await writeJsonFile(filePath, todos);
        return {
          content: [{ type: 'text' as const, text: `Task "${todos[idx].title}" updated` }],
        };
      } catch (err) {
        return {
  ...
  server.tool(
    ...
    async ({ id }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Task deleted` }],
        };
      } catch (err) {
        return {
