...
export function registerThoughtTools(server, projectPath) {
    ...
    server.tool('thought_list', 'List all recorded thoughts', {}, async () => {
        try {
            ...
            return {
                content: [{ type: 'text', text: `Error listing thoughts: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool('thought_add', 'Add a new thought entry', {
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: `Error recording thought: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool('project_info', 'Read the project info/context from info.md', {}, async () => {
        try {
            ...
            return {
                content: [{ type: 'text', text: `Error reading project info: ${err}` }],
                isError: true,
            };
        }
    });
