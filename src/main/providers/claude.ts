...
function resolveClaudeCliPath(): string {
  const candidates = [
    ...
    path.join(__dirname, '..', '..', 'node_modules', '@anthropic-ai', 'claude-agent-sdk', 'cli.js'),
  ];
...
class MessageQueue {
  ...
  private resolve: ((val: IteratorResult<SDKUserMessage>) => void) | null = null;
  private done = false;
...
function transformMessage(msg: SDKMessage): unknown | null {
  if (msg.type === 'assistant') {
    ...
    for (const block of msg.message?.content ?? []) {
      if (block.type === 'text') {
        ...
        blocks.push({ type: 'tool_use', id: block.id, tool: block.name, input: block.input });
      } else if (block.type === 'thinking') {
        blocks.push({ type: 'thinking', text: (block as { text?: string }).text ?? '' });
      }
    }
    return blocks;
  }
  if (msg.type === 'tool_progress') {
    return { type: 'tool_progress', tool: msg.tool_name, elapsedSeconds: msg.elapsed_time_seconds };
  }
  if (msg.type === 'result') {
    return {
      type: 'result',
      ...
      errors: msg.subtype !== 'success' ? (msg as { errors?: string[] }).errors : undefined,
    };
  }
  ...
  if (msg.type === 'user' && 'tool_use_result' in msg && msg.tool_use_result) {
    return {
      type: 'tool_result',
      output: typeof msg.tool_use_result === 'string' ? msg.tool_use_result : JSON.stringify(msg.tool_use_result),
    };
  }
  return null;
}
...
function emitMessage(session: ClaudeSession, msg: SDKMessage) {
  ...
  if (msg.type === 'tool_progress') {
    ...
    for (const block of transformed) session.send(session.messageChannel, block);
  } else {
...
function loadProjectMcpServers(projectPath: string): Record<string, unknown> {
  try {
    const raw = fs.readFileSync(path.join(projectPath, '.nightpm', 'mcp.json'), 'utf-8');
    return JSON.parse(raw).servers || {};
  } catch { return {}; }
}

function loadProjectAgents(projectPath: string): Record<string, unknown> | undefined {
  try {
    return JSON.parse(fs.readFileSync(path.join(projectPath, '.nightpm', 'agents.json'), 'utf-8'));
...
export function createClaudeProvider(getConfig: () => ClaudeConfig): AIProvider {
  return {
    id: 'claude',
    displayName: 'Claude',
    ...
    async startSession(opts: StartSessionOpts) {
      ...
      const mergedMcpServers: Record<string, unknown> = { 'night-pm': mcpServer, ...projectMcpServers };

      const q = query({
        prompt: inputQueue as unknown as AsyncIterable<SDKUserMessage>,
        options: {
          systemPrompt: opts.systemInstructions
            ? { type: 'preset', preset: 'claude_code', append: opts.systemInstructions }
          ...
          stderr: (data: string) => { console.error(`[claude:${opts.key}:stderr]`, data); },
        },
      });

      const iterator = q[Symbol.asyncIterator]();
      const session: ClaudeSession = {
        iterator, inputQueue, abortController,
        isRunning: false,
        send: opts.send,
    ...
    async sendFollowup(key, text, send, messageChannel, progressChannel, doneChannel) {
      ...
      session.inputQueue.enqueue(makeUserMessage(text, session.sessionId ?? ''));
      await drainUntilResult(session);
    },
