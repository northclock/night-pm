...
interface DisplayIdea extends Idea {
  _color?: string;
  _projectName?: string;
}
...
export function IdeasView({ file }: Props) {
  ...
  } = useChildProjectData<Idea>(file.path, 'ideas.json');
    ...
    <div className="h-full flex flex-col p-4 overflow-auto">
      ...
      {allIdeas.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {allIdeas.map((idea) => {
            const isChild = childItemIds.has(idea.id);
            return (
              <Card
                key={`${idea._projectName ?? 'own'}-${idea.id}`}
                className="group cursor-pointer hover:border-primary/30 transition-colors"
                style={idea._color ? { borderLeftWidth: 3, borderLeftColor: idea._color } : undefined}
                onClick={() => { if (!isChild) { setEditingIdea(idea); setShowForm(true); } }}
              >
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <CardTitle className="text-sm">
                      {idea._projectName && <span className="text-[11px] font-semibold mr-1.5" style={{ color: idea._color }}>{idea._projectName}:</span>}
                      {idea.title}
                    </CardTitle>
                    {!isChild && (
