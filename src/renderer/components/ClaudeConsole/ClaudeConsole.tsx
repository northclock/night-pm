...
interface LogEntry { id: number; role: 'user' | 'assistant' | 'system'; blocks: ClaudeMessage[]; }
interface ClaudeConsoleProps { onClose: () => void; }
let logId = 0;

export function ClaudeConsole({ onClose }: ClaudeConsoleProps) {
  ...
  useEffect(() => {
    const cleanups = [
      window.nightAPI.claude.onConsoleMessage((msg: ClaudeMessage) => {
        setEntries((prev) => {
      ...
      window.nightAPI.claude.onConsoleProgress((msg: ClaudeMessage) => {
        setEntries((prev) => {
          const last = prev[prev.length - 1];
          if (last?.role === 'assistant') {
            return [...prev.slice(0, -1), { ...last, blocks: [...last.blocks, msg] }];
      ...
      window.nightAPI.claude.onConsoleDone((result: ClaudeResult) => {
        setIsRunning(false);
  ...
  const loadSessions = useCallback(async () => {
    const s = await window.nightAPI.claude.listSessions();
    setSessions(s);
  }, []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (hasConversation) {
      await window.nightAPI.claude.consoleFollowup(text);
    } else {
      await window.nightAPI.claude.consoleRun(text);
  ...
  function handleNewSession() {
    ...
    setLastResult(null);
    window.nightAPI.claude.consoleAbort();
  }
    ...
    <div className="relative h-full flex flex-col overflow-hidden bg-background">
      ...
      <div className="relative z-10 flex items-center justify-between px-4 py-2.5 border-b border-border/50 bg-sidebar/40 backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-foreground">Claude Console</span>
        ...
        <div className="flex items-center gap-1">
          ...
          {isRunning && (
            <Button variant="destructive" size="xs" className="text-[10px] gap-1" onClick={() => { window.nightAPI.claude.consoleAbort(); setIsRunning(false); }}>
              <Stop size={10} /> Stop
            </Button>
          )}
          ...
          {sessions.map((s) => (
            <button
              ...
              onClick={async () => {
                ...
                setHasConversation(true);
                setIsRunning(true);
                await window.nightAPI.claude.resumeSession(s.sessionId);
              }}
            >
        ...
        <div className="px-4 py-3 space-y-3">
          {entries.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-3 text-muted-foreground/50 py-20">
              <Terminal size={28} />
              <p className="text-xs text-center">Send prompts to Claude with full Claude Code capabilities.<br />Multi-turn conversation with project context.</p>
            </div>
          )}
          {entries.map((entry) => (
            <div key={entry.id}>
...
function ConsoleBlock({ block }: { block: ClaudeMessage }) {
