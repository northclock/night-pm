...
export function TodoItem({ todo, onToggle, onEdit, onDelete }: TodoItemProps) {
  ...
  return (
    <div className={`group flex items-start gap-3 p-3 rounded-lg ${config.bg} hover:brightness-110 transition-all`}>
      <button onClick={onToggle} className={`mt-0.5 ${config.color}`}>
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={onEdit}><PencilSimple size={13} /></Button>
        <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={onDelete}><Trash size={13} /></Button>
      </div>
    </div>
  );
}
