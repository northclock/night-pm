...
contextBridge.exposeInMainWorld('nightAPI', {
  app: {
    setActiveProject: (p: string) => ipcRenderer.invoke('app:setActiveProject', p),
  },
  window: {
    minimize: () => ipcRenderer.invoke('window:minimize'),
    maximize: () => ipcRenderer.invoke('window:maximize'),
    close: () => ipcRenderer.invoke('window:close'),
    onFullscreenChanged: (cb: (v: boolean) => void) => onIpc('window:fullscreen-changed', cb as (...a: unknown[]) => void),
  },
  settings: {
  ...
  ai: {
    ...
    thoughtFollowup: (text: string) => ipcRenderer.invoke('ai:thought-followup', text),
    abort: () => ipcRenderer.invoke('ai:abort'),
    consoleRun: (cmd: string) => ipcRenderer.invoke('ai:console-run', cmd),
    consoleFollowup: (text: string) => ipcRenderer.invoke('ai:console-followup', text),
    ...
    onConsoleMessage: (cb: (msg: unknown) => void) => onIpc('ai:console-message', cb),
    onConsoleProgress: (cb: (msg: unknown) => void) => onIpc('ai:console-progress', cb),
    onConsoleDone: (cb: (result: unknown) => void) => onIpc('ai:console-done', cb),
  },
  fs: {
    ...
    createFile: (p: string, c: string) => ipcRenderer.invoke('fs:createFile', p, c),
    ...
    onFileChanged: (cb: (path: string, content: string) => void) => {
      const handler = (_e: Electron.IpcRendererEvent, p: string, c: string) => cb(p, c);
  ...
  project: {
    scaffold: (p: string, n: string) => ipcRenderer.invoke('project:scaffold', p, n),
  },
});
